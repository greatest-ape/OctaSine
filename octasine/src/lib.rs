...
mod tests {
    ...
    fn gen_voice_samples(
        ...
        let time_per_sample = FmSynth::time_per_sample(SampleRate(
            SAMPLE_RATE as f32));
    ...
    /// 
    /// Remember to activate opt-level 3, LTO and target-cpu compiler flag when
    /// running.
    /// 
    /// It would be nice if this were a real #[bench] benchmark, or any other
    /// proper solution.
    #[cfg(feature = "simd")]
    #[allow(dead_code)]
    //#[test]
    fn benchmark_gen_voice_samples(){
        ...
        assert!(false); // We want to print the benchmark, right? :-)
    }
}