...
use plotlib::style::{LineStyle, PointStyle, PointMarker};
...
fn plot_envelope_stage(
    ...
    }, 0., length).line_style(LineStyle::new().colour("green"));
...
fn plot_lfo_values(filename: &str){
    ...
    let release_key_at_samples: Vec<usize> = press_key_at_samples.iter()
    ...
    let mut processing_parameter_envelope = ProcessingParameterOperatorEnvelope::new();
    ...
    let value_product_points = lfo_value_points.iter()
        .zip(envelope_value_points.iter())
        .map(|((lfo_x, lfo_y), (_, envelope_y))| {
            (*lfo_x, lfo_y * envelope_y)
        })
        .collect::<Vec<(f64, f64)>>();

    let value_product_points_nonzero_base = lfo_value_points.iter()
        .zip(envelope_value_points.iter())
        .map(|((lfo_x, lfo_y), (_, envelope_y))| {
            (*lfo_x, (1.0 + lfo_y) * envelope_y)
        })
        .collect::<Vec<(f64, f64)>>();

    let seconds_plot = Plot::new(seconds_points)
        .point_style(PointStyle::new().marker(PointMarker::Circle).colour("#aaa").size(1.0));
    ...
    let lfo_values_plot = Plot::new(lfo_value_points)
        .line_style(LineStyle::new().colour("green").width(0.1));

    let envelope_values_plot = Plot::new(envelope_value_points)
        .line_style(LineStyle::new().colour("blue").width(0.1));
